...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='66049e3b21efe110454d67df4fa62b08ea79a19b'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'HEAD, tag: v1.14.2, origin/release-1.14' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'HEAD, tag: v1.14.2, origin/release-1.14' =~ tag:\ (v[^ ,]+) ]]; then
